...
int main(int argc, char *argv[]) {
  ...
  while (running) {
    ...
    } else if (mqtt_client_is_connected(&config)) {
      // Publish only if connected
      if (mqtt_client_publish_metric(&config, json) == 0) {
        message_count++;
        printf("[%d] Published: temp=%.2f°C, vib=%.4fg\n", message_count, temp,
               vib);
